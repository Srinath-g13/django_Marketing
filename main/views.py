...
def itemspage(request):
    ...
    if request.method =='POST':
        ...
        if purchased_item:
            ...
            messages.success(request,f'The product {purchased_item_object.name} has been bought for {purchased_item_object.price}')
...
def registerpage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, f'Registered successfully! Logged in as {user.username}')
